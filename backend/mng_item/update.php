<?php
/**
 * PUT /backend/mng_item/update.php
 * 세부 항목 수정
 */
...
try {
    ...
    // 대상 존재 확인
    $existing = $itemSQL->findById($id);
    if (!$existing) {
        Response::error('존재하지 않는 세부 항목입니다.');
    }

    // 코드 변경 시 동일 대항목 내 중복 검사
    if ($existing['code'] !== $code) {
        if ($itemSQL->findByCategoryAndCode($existing['category_id'], $code)) {
            Response::error('이 대항목 내에 이미 동일한 세부 항목 코드가 등록되어 있습니다.');
        }
    }

    $success = $itemSQL->update($id, [
        'code'        => $code,
        'name'        => $name,
        'is_use'      => $isUse,
        'description' => $desc
    ...
    if ($success) {
        Response::success(null, '세부 항목이 성공적으로 수정되었습니다.');
    } else {
